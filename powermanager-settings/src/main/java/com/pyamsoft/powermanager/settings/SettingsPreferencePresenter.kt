...
internal class SettingsPreferencePresenter @Inject constructor(
  ...
  fun clearAll(callback: ClearRequestCallback) {
    disposeOnStop(interactor.clearAll()
        .subscribeOn(subscribeScheduler)
        .observeOn(observeScheduler)
        .subscribe({ callback.onClearAll() }, { Timber.e(it, "onError") }))
  }
  ...
  fun clearDatabase(callback: ClearRequestCallback) {
    disposeOnStop(interactor.clearDatabase()
        .subscribeOn(subscribeScheduler)
        .observeOn(observeScheduler)
        .subscribe({ callback.onClearDatabase() }, { Timber.e(it, "onError") }))
  }
  ...
  fun checkRootEnabled(callback: RootCallback) {
    disposeOnStop(interactor.isRootEnabled
        ...
        .subscribe({ checkRoot(false, it, callback) }, { Timber.e(it, "onError bindCheckRoot") }))
  }
  ...
  fun checkRoot(causedByUser: Boolean, rootEnable: Boolean, callback: RootCallback) {
    disposeOnStop(interactor.checkRoot(rootEnable)
        ...
        .doAfterTerminate { callback.onComplete() }
        .doOnSubscribe { callback.onBegin() }
        .subscribe({ callback.onRootCallback(causedByUser, it, rootEnable) }
            , {
