...
public class SyncDisableTimePreference extends CustomTimeInputPreference {
  @Inject @Named("sync_custom_disable") CustomTimeInputPreferencePresenter presenter;

  public SyncDisableTimePreference(Context context, AttributeSet attrs, int defStyleAttr,
      int defStyleRes) {
  ...
  public SyncDisableTimePreference(Context context, AttributeSet attrs) {
    super(context, attrs);
  }

  public SyncDisableTimePreference(Context context) {
    super(context);
  }

  @NonNull @Override protected CharSequence formatSummaryStringForTime(long time) {
    return String.format(Locale.getDefault(), "Current Sync disable time period: %d seconds", time);
  }
