...
public class AirplaneEnableTimePreference extends CustomTimeInputPreference {

  @SuppressWarnings("WeakerAccess") @Inject @Named("airplane_custom_enable")
  CustomTimeInputPreferencePresenter presenter;

  public AirplaneEnableTimePreference(Context context, AttributeSet attrs, int defStyleAttr,
      int defStyleRes) {
  ...
  public AirplaneEnableTimePreference(Context context, AttributeSet attrs) {
    super(context, attrs);
  }

  public AirplaneEnableTimePreference(Context context) {
    super(context);
  }

  @NonNull @Override protected CharSequence formatSummaryStringForTime(long time) {
    return String.format(Locale.getDefault(), "Current Airplane Mode enable time period: %d seconds",
        time);
  }
